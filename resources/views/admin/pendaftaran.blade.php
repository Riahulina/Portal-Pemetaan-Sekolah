        ...
        <div x-show="filterOpen" x-transition class="bg-white border border-gray-200 rounded-xl p-5 mb-6">
            <div class="grid grid-columns-1 md:grid-cols-5 gap-4 items-end">
                <div>
                    ...
                    <select class="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs text-gray-500 bg-white focus:outline-none focus:border-[#0d9296]">
                        ...
                        @forelse ($pendaftaran as $row)
                            <tr class="hover:bg-gray-50/70 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @endphp
                                    <span class="inline-block px-3 py-0.5 text-xs font-bold rounded {{ $badgeClass }}">{{ $jenjang }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    ...
                                    <p class="text-gray-500 text-sm">Tidak ada pendaftaran baru yang menunggu verifikasi.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-1">
                            @if ($pendaftaran->onFirstPage())
                                <span class="px-2.5 py-1 text-gray-300 bg-gray-50 border border-gray-200 rounded cursor-not-allowed">&lt;</span>
                            @else
                                <a href="{{ $pendaftaran->previousPageUrl() }}" class="px-2.5 py-1 text-gray-600 bg-white border border-gray-200 rounded hover:bg-gray-50 transition-colors">&lt;</a>
                            @endif

                            @foreach ($pendaftaran->getUrlRange(1, $pendaftaran->lastPage()) as $page => $url)
                                @if ($page == $pendaftaran->currentPage())
                                    <span class="px-2.5 py-1 text-white bg-[#0d9296] rounded font-bold">{{ $page }}</span>
                                @else
                                    <a href="{{ $url }}" class="px-2.5 py-1 text-gray-600 bg-white border border-gray-200 rounded hover:bg-gray-50 transition-colors">{{ $page }}</a>
                                @endif
                            @endforeach

                            @if ($pendaftaran->hasMorePages())
                                <a href="{{ $pendaftaran->nextPageUrl() }}" class="px-2.5 py-1 text-gray-600 bg-white border border-gray-200 rounded hover:bg-gray-50 transition-colors">&gt;</a>
                            @else
                                <span class="px-2.5 py-1 text-gray-300 bg-gray-50 border border-gray-200 rounded cursor-not-allowed">&gt;</span>
                            @endif
                        </div>
                        ...
                        <!-- Rows Per Page -->
                        <div class="flex items-center gap-1">
                            <select class="px-2 py-1 border border-gray-200 rounded text-gray-600 bg-white focus:outline-none">
